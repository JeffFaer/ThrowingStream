...
class UnionFunctionAdapter<X extends UnionThrowable> {
    ...
    public ThrowingIntUnaryOperator<X> convert(
            ThrowingIntUnaryOperator<? extends Throwable> operator) {
        ThrowingIntUnaryOperator<Throwable> o = operator::applyAsInt;
        return o.rethrow(x, mapper);
    }

    public <R> ThrowingIntFunction<R, X> convert(
            ThrowingIntFunction<R, ? extends Throwable> function) {
